...
final public class BSONSerialization {
	...
	public struct WritingOptions : OptionSet {
		...
		/**
		 Set all sizes to 0 in generated BSON.
		 Mainly useful internally for optimization purposes. */
	...
	/**
	 ...
	 - Parameter options: Some options to customize the parsing. See ``ReadingOptions``.
	...
	/**
	 ...
	 Exactly the size of the BSON document will be read from the stream.
	 If an error occurs while reading the BSON document, you are guaranteed that less than the size of the BSON doc is read.
	 If the size of the BSON declared in the stream is invalid, the read bytes count is undetermined.
	...
	class func bsonObject(with streamReader: StreamReader, options opt: ReadingOptions = [], initialReadPosition: Int? = nil, decodeCallback: (_ key: String, _ val: Any?) throws -> Void = {_,_ in}) throws -> BSONDoc {
		...
		while !isAtEnd {
			...
			switch BSONElementType(rawValue: currentElementType) {
				...
				case .double128Bits?:
					/* Note: We assume Swift will **always** represent tuples the way it currently does and struct won’t have any padding… */
		...
		if !opt.contains(.skipSizes) {
			data.withUnsafeMutableBytes{ (bytes: UnsafeMutableRawBufferPointer) -> Void in
				...
				for (offset, size) in sizes {
					/* We can either bind the memory, or assume it’s already bound, both cases are “working” (well the tests pass).
					 * Justification: We know the data won’t be aliased as we will modify and access it in this method only.
					 * I prefer to bind as the doc says
					 *    “Any typed memory access, either via a normal safe language construct or via ${Self}<T>,
					 *     requires that the access type be compatible with the memory’s currently ‘bound’ type.”
					 ...
					 * For a trivial type I don’t think it matters, but still, let’s do the things “the right way.” */
					(baseAddress + offset).bindMemory(to: type(of: size), capacity: 1).pointee = size
					/* Note: Ideally I’d love to be able to do the line below, but currently it can crash with “storeBytes to misaligned raw pointer” (which is expected as per the documentation).
					 * Joe said there should be a version of storeBytes in the stdlib able to store at unaligned locations, but it is not there yet.*/
//					(baseAddress + offset).storeBytes(of: size, as: type(of: size))
				}
			}
			/* A variant of the code above.
			 * While this variant is “safer” because we know we won’t have memory binding problems, it is MUCH slower.*/
...
//			}
	...
	/**
	 ...
	 If a BSON doc contains an `NSNumber` representing the `Int` 0, Swift will happily cast it to a `Bool` containing `false`.
	 
	 This can be a problem.
	 When switching on the entity being encoded to find its type, we can encode an `Int` as a `Bool`
	 (or vice versa depending on the order of the switch cases).
	 
	 So we “normalize” the input entity by converting an `NSNumber` to its encoded type.
	 
	 For the time being we only have this normalization done; we might find more later. */
	...
	private enum BSONElementType : UInt8 {
		...
		/** `nil` */
		case null                = 0x0A
		/** `Bool`. Raw value is a single byte, containing `0` (`false`) or `1` (`true`). */
		case boolean             = 0x08
		...
		/** `Date`. Raw value is the number of milliseconds since the Epoch in UTC in an Int64. */
		case utcDateTime         = 0x09
		/** `RegularExpression`. Raw value is two cstring: Regexp pattern first, then regexp options. */
		case regularExpression   = 0x0B
		
		/** `String`. Raw value is an Int32 representing the length of the string + 1, then the actual bytes of the string, then a single byte equal to `0`. */
		case utf8String          = 0x02
		
		/** `BSONDoc`. Raw value is an embedded BSON document. */
		case dictionary          = 0x03
		/** `[Any?]`. Raw value is an embedded BSON document; keys are "0", "1", etc. and must be ordered in numerical order. */
		case array               = 0x04
		
		/**
		 `.MongoTimestamp`. Special internal type used by MongoDB replication and sharding.
		 First 4 bytes are an increment, second 4 are a timestamp. */
		case timestamp           = 0x11
		/** `.MongoBinary`. Raw value is an Int32, followed by a subtype (1 byte) then the actual bytes. */
		case binary              = 0x05
		...
		/**
		 `.JavascriptWithScope`.
		 Raw value is an Int32 representing the length of the whole raw value, then a string, then an embedded BSON doc.
		...
		/** `.MinKey` Special type which compares lower than all other possible BSON element values. */
		case minKey              = 0xFF
		/** `.MaxKey` Special type which compares higher than all other possible BSON element values. */
		case maxKey              = 0x7F
		
		/** `nil`. Undefined value. Deprecated. */
		case undefined           = 0x06
		/** `.MongoDBPointer`. Deprecated. Raw value is a string followed by 12 bytes. */
		case dbPointer           = 0x0C
	...
	/**
	 ...
	 - Note: The .skipSizes option is ignored (determined by whether the sizes argument is `nil`).
	 
	 - Note: When possible, `sizeFoundCallback` should be optional (cannot be a non-escaped closure in current Swift state). */
	...
	private class func write(BSONObject: BSONDoc, toStream stream: OutputStream, options opt: WritingOptions, initialWritePosition: Int, sizes knownSizes: UnsafeMutablePointer<[Int]>?, sizeFoundCallback: (_ offset: Int, _ size: Int32) -> Void) throws -> Int {
		...
		/* If skipping sizes, we have to call the callback for size found (the doc is written entirely, we now know its size!) */
...
private extension StreamReader {
	...
	func readBSONString(encoding: String.Encoding) throws -> String {
		 ...
		 * Note: We use the copying version of the simple read stream protocol.
		 *       We could use the version which gives access to the raw buffer pointer instead
		 *       and avoid a copy when the string data is valid for the given encoding,
